...
students       <- read.csv(paste(indir, "puntajes.csv", sep=""), sep=',', header=TRUE)
inscritos      <- read.csv(paste(indir, "inscritos_y_sexo.csv", sep=""), sep=';', header=TRUE)
pace           <- read.csv(paste(indir, "raw/adm2022_pace_habilitados_2022ene10_MRUN.csv", sep=""), sep=',', header=TRUE)
```

```{r}
id_sexo <- read_dta(file="/Users/iriosu/Dropbox/Mistakes/Code/Python/Cartillas/data/2022/raw/archivo C 2022 01072022/id_sexo.dta")
```
...
## Merge inscritos y puntajes
```{r}
length(setdiff(students$mrun, inscritos$mrun))
```

RESULTADO: todos los alumnos con puntajes estan en inscritos

## Consistencia de generos 

```{r}
length(setdiff(students$mrun, id_sexo$ID))
length(setdiff(id_sexo$ID, students$mrun ))
```

RESULTADO: NO COINCIDEN LOS IDENTIFICADORES!

## Inscritos y PACE

```{r}
length(setdiff(pace$mrun, inscritos$mrun))
```

RESULTADO: todos los alumnos PACE estan en inscritos

## Puntajes y PACE

```{r}
length(setdiff(pace$mrun, students$mrun))
```

RESULTADO: 5231 estudiantes PACE no estan en puntajes. Esto es posible ya que no todos los PACE dan la prueba.
...
```{r}
df <- merge(pace, students %>% dplyr::select(mrun, ptje_nem, ptje_ranking, rbd, codigo_region, pace), by="mrun")
```

```{r}
df %>% filter(nem != ptje_nem | rkg != ptje_ranking | rbd.x != rbd.y | region_egreso != codigo_region) %>% nrow()
```

```{r}
df %>% group_by(pace) %>% tally()
```

RESULTADO: no existen discrepancias en los datos.


# Inspeccion visual de puntajes
```{r}
hist(students$ptje_nem)
hist(students$ptje_ranking)
hist(students$mate_actual)
hist(students$clec_actual)
hist(students$porc_sup_notas)
```

```{r}
students %>% group_by(bea) %>% tally()
students %>% group_by(pace) %>% tally()
```


RESULTADO: pareciera estar ok.
...
```{r}
acp <- read.csv("/Users/IgnacioRios/Dropbox/Mistakes/Code/Python/Cartillas/data/2022/accesos-cartilla-postulaciones.csv", header=TRUE, sep=',')
```

```{r}
acp %>% nrow()
acp %>% distinct(distinct_id) %>% nrow()
acp %>% group_by(distinct_id) %>% tally() %>% arrange(-n)
```
...
```{r}
ins <- read.csv("/Users/iriosu/Dropbox/Mistakes/Code/Python/Cartillas/data/2022/raw/ArchivoB_2022_MRUN_nombre.csv",header=TRUE, sep=';')
```

```{r}
ins %>% separate(nombres, c("primer", "segundo"), " ")
```
...
```{r}
students %>% filter(mate_actual > 0 & cien_actual > 0) %>% nrow()
```
