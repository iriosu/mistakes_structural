...
```{r}
rdat <- merge(rdat, dfs_2019[,c("ID", "PACE")], by="ID", all.x=TRUE)
```
...
```{r}
aux <- rbind( car19 %>% dplyr::select(CODIGO, PROM_MIN_LENG_MATE) %>% mutate(val = "Min. Avg. Math/Verbal") %>% dplyr::rename(score=PROM_MIN_LENG_MATE),
              car19 %>% dplyr::select(CODIGO, PTJE_PONDERADO_MINIMO) %>% mutate(val = "Min. Weighted. Score") %>% dplyr::rename(score=PTJE_PONDERADO_MINIMO) )
```



```{r}
ggplot(aux, aes(score, colour=val)) + stat_ecdf(geom = "step", pad = TRUE) +
...
ggsave("plots/cdf_min_scores.pdf")
```
...
```{r}
ggplot(rdat %>% filter(all_mistakes == 1 & is.na(PACE)) ) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(),
        legend.title = element_blank())
# ggsave("plots/distribution_scores_all_mistakes_no_pace.pdf")
```
...
```{r}
rdat %>% filter(all_mistakes == 1 & is.na(PACE)) %>% group_by(plm >= 450) %>% summarise(n_distinct(ID))
```
